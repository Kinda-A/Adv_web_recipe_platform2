...
<nav class="navbar navbar-dark px-4 py-3 d-flex justify-content-between">
    ...
    <div class="d-flex gap-2 align-items-center">
        ...
        <!-- SEARCH -->
        <form method="GET" action="/recipes" class="d-flex gap-1">
...
<div class="container py-5">
    ...
    <a href="/recipes/{{ $recipe->id }}" class="btn btn-beige btn-sm">
        View
    </a>

    <a href="{{ route('recipes.edit', $recipe->id) }}"
       class="btn btn-beige btn-sm"
       onclick="return checkOwner({{ auth()->id() ?? 'null' }}, {{ $recipe->user_id }})">
        Edit
    </a>

    <form action="{{ route('recipes.destroy', $recipe->id) }}" method="POST"
          onsubmit="return checkOwner({{ auth()->id() ?? 'null' }}, {{ $recipe->user_id }}) && confirm('Delete this recipe?')">
...
<script>
function checkOwner(userId, ownerId) {
    ...
    if (userId !== ownerId) {
        alert("You can't do this — it's not your recipe!");
        return false;
    }
